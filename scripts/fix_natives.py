...
def main() -> None:
    ...
    # better-sqlite3 — Electron 42 / Node ABI 146. 12.6.2 in the payload has
    # no linux-electron prebuild; 12.11.1 ships a compatible v146 binary.
    bs_url = (
    ...
    tmp = Path(tempfile.mkdtemp(prefix="gb-bs-"))
    tgz = tmp / "bs.tar.gz"
    download(bs_url, tgz)
    with tarfile.open(tgz, "r:gz") as tf:
        tf.extractall(tmp / "ex")
    ...
    # tree-sitter 0.21.1 (node-gyp-build → prebuilds/linux-x64)
    ...
    # whichlang-node 0.2.1 — napi-rs loader wants whichlang-node.linux-x64-gnu.node
    wl_pkg = npm_pack_extract(
        "whichlang-node-linux-x64-gnu@0.2.1", tmp / "whichlang"
    )
    wl_node = next(wl_pkg.glob("*.linux-x64-gnu.node"))
    shutil.copy2(wl_node, DEPS / "whichlang-node" / wl_node.name)
